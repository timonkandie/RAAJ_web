...
function createServiceCard({ icon = 'ðŸŽ¨', title = '', description = '', software = [], linkUrl = 'services.html', ctaText = 'Learn More' }) {
...
<a href="${linkUrl}" class="service-card-link">${ctaText} â†’</a>
</div>
</div>
`.trim();
}
...
const icon = container.dataset.icon || 'ðŸŽ¨';
const title = container.dataset.title || '';
...
ModalEngine.open({
title: `${title} â€” Project Case Study`,
size: 'lg',
content: `
...
<a href="contact.html" class="btn btn-primary btn-block">Inquire Similar Project â†’</a>
</div>
</div>
`
});
...
const starsHTML = 'â˜…'.repeat(Math.min(5, Math.max(1, rating))) + 'â˜†'.repeat(5 - Math.min(5, Math.max(1, rating)));
...
<a href="${postUrl}" class="service-card-link">Read Article â†’</a>
</div>
</div>
`.trim();
}
